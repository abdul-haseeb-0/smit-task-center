# ReadyFlight : AI agentic conversation system
...
from agents import Agent, Runner, RunContextWrapper, function_tool, handoff, OpenAIChatCompletionsModel
...
# db_client = get_mongodb_uri()
# db = db_client["readyflight"]
...
async def flight_status_overview_tool(context: RunContextWrapper[AirlineAgentContext]) -> str:
    ...
    for flight_num, flight in flights_db.items():
        ...
        flight_bookings = sum(1 for b in bookings_db.values() if b["flight_number"] == flight_num and b["status"] == "confirmed")
